  ...
  return (
    <div className="w-full shadow-navShadow h-20 sticky top-0 z-50 bg-bodyColor pr-4 lg:h-[12vh] hover:shadow-navbarShadow">
      <div className="max-w-container h-full mx-auto py-1 font-inter flex items-center justify-between">
        ...
        >
          <span
            className={`w-full h-[2px] bg-textGreen transition-all duration-300 ${isOpen ? "rotate-45 translate-y-2" : ""}`}
          ></span>
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="fixed inset-0 bg-black/60 backdrop-blur-lg flex flex-col items-center justify-center z-50 mdl:hidden"
            >
              <motion.div
                ...
                className="w-3/4 bg-bodyColor shadow-xl rounded-lg p-8 text-center"
              >
                <ul className="flex flex-col gap-6 text-xl text-textDark">
                  {["home", "about", "experience", "projects", "contact"].map((section, index) => (
                    ...
                    >
                      {index > 0 ? <span className="text-textGreen">0{index}.</span> : <span className="ml-6"></span>}{" "}
                      {section.charAt(0).toUpperCase() + section.slice(1)}
                    </motion.li>
                  ))}
                </ul>
                ...
                <button
                  ...
                  className="absolute top-6 right-6 text-3xl text-textGreen"
                >
                  ✕
                </button>
              </motion.div>
